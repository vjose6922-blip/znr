...
_miniSliderSt.textContent = '.mini-slider{position:relative;width:100%;height:100%;overflow:hidden;touch-action:pan-y;}.mini-slider-track{display:flex;width:100%;height:100%;transition:transform .3s ease;}.mini-slider-slide{min-width:100%;height:100%;flex-shrink:0;}.mini-slider-dots{position:absolute;bottom:6px;left:50%;transform:translateX(-50%);display:flex;gap:4px;z-index:2;}.mini-slider-dot{width:5px;height:5px;border-radius:50%;background:rgba(255,255,255,.6);box-shadow:0 0 2px rgba(0,0,0,.4);transition:all .2s;}.mini-slider-dot.active{background:var(--color-accent-solid);width:12px;border-radius:3px;}';
...
.im-vendor-pill-znr {
  padding: 6px 14px; 
  background: linear-gradient(135deg,var(--color-accent-solid),#a855f7);
}
.im-donation-badge {
...
.im-close:hover { background: rgba(var(--color-accent-rgb),0.22); }
...
.im-prev:hover, .im-next:hover { background: rgba(var(--color-accent-rgb),0.22); }
...
.im-dot.active { background: var(--color-accent-solid); width: 16px; border-radius: 3px; }
...
.im-related-card:hover { background: rgba(var(--color-accent-rgb),0.08); }
...
.im-related-card:hover::after { border-color: rgba(var(--color-accent-rgb),0.22); }
...
.im-related-price { font-size: 12px; font-weight: 800; color: var(--color-accent-solid); margin-top: 1px; }
...
[data-theme="light"] .im-close:hover { background: rgba(var(--color-accent-rgb),0.15); color: #e11d6a; }
...
[data-theme="light"] .im-prev:hover, [data-theme="light"] .im-next:hover { background: rgba(var(--color-accent-rgb),0.22); color: #fff; }
...
[data-theme="light"] .im-related-card:hover { background: rgba(var(--color-accent-rgb),0.06); }
...
style="padding:6px 10px;border-radius:20px;border:1.5px solid ${active?'var(--color-accent-solid)':'rgba(255,255,255,.15)'};
background:${active?'rgba(var(--color-accent-rgb),0.18)':'transparent'};color:${active?'var(--color-accent-solid)':'#aaa'};
...
<button id="_addr-confirm" style="flex:2;padding:13px;background:var(--gradient-accent);border:none;border-radius:14px;color:#fff;font-size:14px;font-weight:700;cursor:pointer"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" aria-hidden="true"><use href="#ic-check"/></svg> Confirmar</button>
</div>
</div>
`;
...
btn.style.border = '1.5px solid var(--color-accent-solid)'; btn.style.background = 'rgba(var(--color-accent-rgb),0.18)'; btn.style.color = 'var(--color-accent-solid)';
}
});
}
...
mapsLink.style.cssText = 'display:inline-flex;align-items:center;gap:5px;font-size:12px;color:var(--color-accent-solid);text-decoration:none;font-weight:600;margin-bottom:16px';
...
btn.style.border  = `1.5px solid ${active ? 'var(--color-accent-solid)' : 'rgba(255,255,255,.15)'}`;
btn.style.background  = active ? 'rgba(var(--color-accent-rgb),0.18)' : 'transparent';
btn.style.color  = active ? 'var(--color-accent-solid)' : '#aaa';
});
});
...
<div style="font-size:13px;font-weight:700;color:var(--color-accent-solid);white-space:nowrap">${formatCurrency(i.price * i.quantity)}</div>
</div>
`).join('');
...
<span style="font-size:18px;font-weight:800;color:var(--color-accent-solid)">${formatCurrency(subtotal)}</span>
</div>
</div>
${entregaHtml}
${donationHtml}
...
background:linear-gradient(135deg,var(--color-accent-solid),#7c3aed);
