  ...
  if (!document.getElementById('im-styles')) {
    ...
    st.textContent = `
      ...
      @media (min-width: 600px) {
        ...
        .im-wrapper img { max-height: 62dvh; }
      }
    `;
...
function attachPanelEvents(panel) {
  ...
  panel.querySelectorAll('.up-layout-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      // También afecta comunidad y home
      ...
      if (comunidadGrid) {
        if (layout==='list') { comunidadGrid.classList.add('layout-list'); }
        else { comunidadGrid.classList.remove('layout-list'); }
        const ltComBtn = document.getElementById('layout-toggle-comunidad');
        if (ltComBtn) ltComBtn.textContent = layout==='list' ? '\u2261' : '\u25a6';
      }
      const homeGrid = document.getElementById('home-comunidad-grid');
      if (homeGrid) {
        if (layout==='list') { homeGrid.classList.add('layout-list'); homeGrid.style.gridTemplateColumns='1fr'; }
        else { homeGrid.classList.remove('layout-list'); homeGrid.style.gridTemplateColumns=''; }
      }
      const featuredGrid = document.getElementById('featured-products');
      if (featuredGrid) {
        if (layout==='list') { featuredGrid.classList.add('layout-list'); }
        else { featuredGrid.classList.remove('layout-list'); }
      }
