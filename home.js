      ...
      if (container) {
        container.innerHTML = `
          <div style="text-align:center;padding:40px;grid-column:1/-1;">
            ...
            <button onclick="loadProducts()" style="padding:10px 24px;border-radius:40px;border:none;background:var(--color-accent,#ff4f81);color:white;font-weight:600;cursor:pointer;font-size:14px;">🔄 Reintentar</button>
          </div>`;
      }
...
function createMiniProductCard(product) {
  ...
  const stockLabel = !hasStock
    ...
    : stockNum <= 3
      ? `<span style="color:var(--color-warning,#f97316);font-size:11px;">⚠️ Últimas ${stockNum}</span>`
...
function setupEventListeners() {
  ...
  // Keep recent-products in sync with global layout changes
