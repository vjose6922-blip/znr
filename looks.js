...
let allProducts = [];
let productsByCategoryIndex = new Map(); // índice O(1) por categoría
let looks = [];
let allLooks = [];
...
function getOutfitTip(classified) {
  ...
  if (condition === 'lluvia' || condition === 'tormenta') return '☂️ Lleva impermeable';
...
function updateWeatherWidgetUI(classified) {
  ...
  widget.innerHTML = `
    ...
    <span class="weather-condition">${classified.condition.replace(/_/g,' ')}</span>
    ...
    <span class="weather-outfit-tip">${getOutfitTip(classified)}</span>
  `;
...
// Pre-indexa productos por categoría para evitar O(n) por slot
...
function getProductsForSlot(products, slot) {
  // Construir candidatos desde el índice (O(1) por categoría) en lugar de filtrar todo
  let candidates;
  if (slot.categories.length === 0) {
    candidates = products; // sin restricción de categoría, usar todo
  } else {
    candidates = [];
...
function createLookCardMini(look) {
  ...
  div.innerHTML = `
    <div class="cart-item-info" style="flex:1">
      ...
      <div class="cart-item-meta" style="font-weight:bold; color:var(--color-accent,#ff4f81); margin: 8px 0;">
        Total: ${formatCurrency(totalPrice)}
      </div>
      <div class="cart-item-actions" style="margin-top: 8px; display: flex; gap: 8px;">
        <button class="add-look-to-cart" data-look-id="${look.id}" style="background:var(--color-info-bg,#e8e8ff); border:none; padding: 6px 12px; border-radius: 20px; cursor: pointer; font-size: 12px;">🛒 Agregar todo</button>
        <button class="remove-look-from-wishlist" data-look-id="${look.id}" style="background:var(--color-error-bg,#ffe8e8); border:none; padding: 6px 12px; border-radius: 20px; cursor: pointer; font-size: 12px;">🗑 Eliminar</button>
      </div>
    </div>
  `;
...
function initLooksLayoutToggle() {
  ...
  // React to global layout changes from settings panel
