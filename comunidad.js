    ...
    try {
      ...
      card.innerHTML = `
        <div class="product-slider" style="position:relative;cursor:pointer;">
          ...
          ${inspectorMode ? '<span style="position:absolute;top:8px;right:8px;z-index:2;"><button class="btn-inspector-delete" title="Eliminar (Admin)" style="background:var(--color-error,#ef4444);color:#fff;border:none;border-radius:20px;padding:2px 8px;font-size:11px;cursor:pointer;">🗑️</button></span>' : ''}
          <img class="product-img-main" src="${esc(imgUrl)}" alt="${esc(safeString(product.nombre))}" loading="lazy"
               style="width:100%;aspect-ratio:1;object-fit:contain;display:block;background:var(--color-surface-2,#f5f5f8);">
        </div>
        <div class="product-info" style="padding:12px;">
          ...
          <div class="product-actions" style="margin-top:8px;display:flex;gap:6px;">
            ...
            <button class="btn-report" title="Reportar" style="background:rgba(0,0,0,.06);border:none;border-radius:30px;padding:8px 10px;cursor:pointer;font-size:13px;color:var(--color-text-muted,#aaa);">🚩</button>
          </div>
        </div>
      `;
