...
async function fetchProducts(force = false) {
  window.loadProductsUnified({
    ...
    onProducts(products, fromCache) {
      ...
      // Si vino de caché y estamos online, el background refresh lo hace loadProductsUnified,
      // pero re-aplicamos filtros si ya hay alguno activo cuando llega la actualización.
    },
    onError() {
...
function applyFilters() {
  ...
  currentPage = 1;
  updateFilterChips({ searchValue, categoryValue, genderValue, sortValue });
  renderProductsPage(true);
}

// ── Chips de filtros activos ──────────────────────────────────
function updateFilterChips({ searchValue, categoryValue, genderValue, sortValue }) {
  ...
  bar.innerHTML = chips.map((c, i) =>
    ...
    ? `<button class="filter-chip filter-chip-clear" data-chip="all">Limpiar todo</button>`
    : '');
  ...
  bar.querySelectorAll('.filter-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (idx === 'all') {
        ['gender-filter','category-filter','sort-select'].forEach(id => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (layoutBtn && productsContainer) {
    ...
    // React to layout changes from settings panel or other pages
