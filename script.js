...
populateSizeFilter(genderValue, categoryValue);
...
(p.Talla || "").toLowerCase().includes(searchValue) ||
(p.Categoria || "").toLowerCase().includes(searchValue) ||
(p.Badge || "").toLowerCase().includes(searchValue);
const matchesTalla = !sizeValue ||
(p.Talla || '').split(/[,\/]/).map(t => t.trim()).includes(sizeValue);
let matchesGender = true;
if (genderValue) {
const productGender = getGenderFromCategory(p.Categoria);
matchesGender = productGender === genderValue;
}
return matchesSearch && matchesGender && matchesTalla;
});
...
function showSkeletonProducts(container, count = 10) {
