...
async function handleAdminLogin(e) {
  ...
  try {
    ...
    localStorage.setItem("admin_token", document.getElementById("admin-token").value);   
...
async function checkNotifications() {
  ...
  try {
    ...
    // El badge principal se actualiza via loadNotifPanelCounts (que suma TODAS las categorías)
...
function getToken() { 
  ...
  function updateReportesBadge(count) {
    ...
    // Siempre sincronizar el tab badge en notificaciones.html
  ...
  async function marcarReporteRevisado(reporteId) {
    try {
      ...
      // Calcular nuevo conteo desde el DOM (más fiable que leer el badge)
      const container = document.getElementById('admin-reportes-list');
