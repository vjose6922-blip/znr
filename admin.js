...
function initAdminViewToggle() {
 ...
 //  CORRECCIÓN: usar la paginación correcta en lugar de mostrar todos
 renderAdminProductsWithFilters();
 }
...
if (search)  products = products.filter(p => (p.Nombre||'').toLowerCase().includes(search) || String(p.ID).includes(search));
if (cat)  products = products.filter(p => p.Categoria === cat);
if (stock === 'out')  products = products.filter(p =>Number(p.Stock||0) === 0);
if (stock === 'low')  products = products.filter(p =>Number(p.Stock||0) > 0 && Number(p.Stock||0) <= 5);
if (stock === 'ok')  products = products.filter(p =>Number(p.Stock||0) > 5);
return products;
}
// 
...
body: JSON.stringify({ action: "verificarAdmin", token: savedToken })
});
const data = await res.json();
...
console.warn("No se pudo verificar sesión admin con el servidor:", e);
adminSession = "ok";
document.getElementById("admin-login-view").hidden = true;
document.getElementById("admin-panel-view").hidden = false;
initImageUploads();
...
try {
const res  = await fetch(getApi(), { method: "POST", headers: {"Content-Type":"application/x-www-form-urlencoded"}, body: new URLSearchParams({ action: "vendedoresAdmin", token: getToken() }).toString() });
const data = await res.json();
...
container.innerHTML = '<p style="color:#aaa;text-align:center">Cargando...</p>';
try {
const res  = await fetch(getApi(), { method: "POST", headers: {"Content-Type":"application/x-www-form-urlencoded"}, body: new URLSearchParams({ action: "productosPendientes", token: getToken() }).toString() });
const data = await res.json();
if (!data.ok) throw new Error(data.error);
...
try {
const res  = await fetch(getApi(), { method: "POST", headers: {"Content-Type":"application/x-www-form-urlencoded"}, body: new URLSearchParams({ action: "obtenerReportes", token: getToken() }).toString() });
const data = await res.json();
if (!data.ok) throw new Error(data.error);
...
prods.innerHTML = '';
fetch(getApi(), { method: "POST", headers: {"Content-Type":"application/x-www-form-urlencoded"}, body: new URLSearchParams({ action: "listarComunidad", vendedor_uid: uid, admin: "true", token: getToken() }).toString() })
.then(r => r.json())
.then(data => {
